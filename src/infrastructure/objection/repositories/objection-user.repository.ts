...
export class ObjectionUserRepository extends UserRepository {
  async create(
    ...
    const created = await UserModel.query(tx).insert(fromUserEntityPatch(user));
  ...
  async update(id: string, patch: Partial<UserEntity>, tx?: Knex.Transaction) {
    const updated = await UserModel.query(tx).patchAndFetchById(
      id,
      fromUserEntityPatch(patch),
    );
